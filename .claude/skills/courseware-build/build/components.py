...
class Deck:
    ...
    # Approximate rendered width, in inches, of a bold Arial string at `pt`.
    # Wrapping depends on rendered WIDTH, not character count — a title of capitals
    # and em dashes is far wider than the same number of narrow letters — so lab
    # slides use this rather than a len() threshold to decide when to shrink.
    _NARROW = set("iljtfrI.,;:'!|()[]{}/\\ ")
    _WIDE = set("MWmw@%—")

    def _text_w(self, txt, pt):
        u = 0.0
        for ch in txt:
    ...
    def _title_wraps(self, title, pt=30, avail_in=11.9):
        """True if `title` will not fit on one line in the standard head box."""
        return self._text_w(title, pt) > avail_in * 0.97
    ...
    def activity_overview(self, tag, title, desc, build, tools, kicker, elective=False):
        # A long lab title wraps to two lines at 30pt and would be clipped by the
        # LAB tag below it, so shrink the title once it gets long.
        if self._title_wraps(title):
            s = self.slide()
            self.rect(s, 0, 0, self.SW, self.SH, WHITE)
            self.rect(s, 0, 0, Inches(0.28), Inches(1.55), TEAL)
            if kicker:
                self.txt(s, Inches(0.85), Inches(0.5), Inches(11.6), Inches(0.4),
                         [[(kicker, 14, TEAL, True)]])
            self.txt(s, Inches(0.85), Inches(0.9), Inches(11.9), Inches(0.82),
                     [[(title, 23, INK, True)]])
        else:
            s = self.head(self.slide(), title, kicker, kcolor=TEAL)
    ...
    def steps_group(self, kicker, act_title, items, first, total):
        ...
        """
        # A long lab title wraps to two lines at 30pt and would collide with the
        # step-count label below it, so shrink the title once it gets long.
        _t = act_title
        if self._title_wraps(_t):
            s = self.slide()
            self.rect(s, 0, 0, self.SW, self.SH, WHITE)
            self.rect(s, 0, 0, Inches(0.28), Inches(1.55), TEAL)
            if kicker:
            ...
            s = self.head(self.slide(), _t, kicker, TEAL)
    ...
    def test_slide(self, act_title, text, kicker):
        # A long lab title wraps to two lines at 30pt and would run into the
        # green check-your-work panel below, so shrink it once it gets long.
        if self._title_wraps(act_title):
            s = self.slide()
            self.rect(s, 0, 0, self.SW, self.SH, WHITE)
            self.rect(s, 0, 0, Inches(0.28), Inches(1.55), TEAL)
            if kicker:
